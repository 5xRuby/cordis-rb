...
- **Service base class(round 4)**:`Cordis::Service` 走 Ruby class DSL —— `provide :name` / `inject :dep`(或 `inject dep: {config}`)取代上游的 `super(ctx, name)` 與 `static inject`;`#initialize(ctx, config)` 內 `ctx.provide(name, self)`,`#init` 對應上游 `Service.init`(在 fiber loading 期間執行,block 住即 pending inject)。registry 支援 Class plugin(對應上游 isConstructor):runtime 以 class 為 key(`Runtime#key` 新欄位,remove 用它刪),callback 包一層 `plugin.new(ctx, config)` + `instance.init`。**跳過**:traceable proxy 相關(`ctx` 重綁、`Service[filter]` isolated event、`[extend]` callable service)、provider-side `check` predicate(上游 spec 沒測,要再說)。無 proxy 所以 per-caller config 要顯式傳 caller ctx 給 service method(見 demo.rb AuditLog)。class ivar 不繼承,巢狀繼承的 provide/inject 不支援(ponytail)。 `/Users/ryudo/RubyPrjs/lens-ruby-async`:完成訊號用 `Async::Queue` 不用 `Async::Condition`(signal 先於 wait 會遺失)、spec 用 `task.with_timeout` 包可能吊死的流程(spec_helper 的 `with_reactor`)、teardown 逐步各自 rescue。
